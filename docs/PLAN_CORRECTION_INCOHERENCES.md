...
### **PHASE 3: Création InstructorService** 🔴 CRITIQUE
**Durée estimée:** 0.5 jour  
**Dépendances:** Phase 1 terminée

#### 3.1. Création du service
- [ ] **Fichier:** `app/Services/InstructorService.php` (nouveau)
  - [ ] Copier méthodes de `BiplaceurService`
  - [ ] Adapter pour utiliser `Instructor` au lieu de `Biplaceur`
  - [ ] Adapter pour utiliser `ActivitySession` au lieu de `Reservation->where('biplaceur_id')`
  - Méthodes à créer:
    - `getSessionsToday(int $instructorId): Collection`
    - `getCalendar(int $instructorId, string $startDate, string $endDate): Collection`
    - `updateAvailability(int $instructorId, array $availability): bool`
    - `markSessionDone(int $sessionId, int $instructorId): ActivitySession`
    - `rescheduleSession(int $sessionId, int $instructorId, string $reason): ActivitySession`
    - `isAvailable(int $instructorId, string $date, string $time = null): bool`

#### 3.2. Tests
- [ ] Créer `tests/Feature/InstructorServiceTest.php`
  - Tester toutes les méthodes avec différents types d'activités
...
### **PHASE 4: Refactorisation Services Spécifiques** 🔴 CRITIQUE
**Durée estimée:** 1 jour  
**Dépendances:** Phases 1, 2, 3 terminées

#### 4.1. StripeTerminalService
- [ ] **Fichier:** `app/Services/StripeTerminalService.php`
  - [ ] Remplacer `Biplaceur` par `Instructor`
  - [ ] Récupérer `can_tap_to_pay` depuis `metadata`
  - [ ] Récupérer `stripe_terminal_location_id` depuis `metadata`
  - [ ] Modifier signature: `getConnectionToken(int $instructorId)`

#### 4.2. VehicleService
- [ ] **Fichier:** `app/Services/VehicleService.php`
  - [ ] Remplacer `biplaceur_id` par `instructor_id`
  - [ ] Utiliser `activitySessions` pour compter participants additionnels
  - [ ] Calculer poids instructeur depuis `metadata`
  - [ ] Modifier méthodes:
    - `countPassengers(Reservation $reservation): int`
    - `checkWeightLimit(int $vehicleId, array $passengers, ?int $instructorId = null): bool`
    - `calculateTotalWeight(Reservation $reservation): float`
    - `calculateNeededSeats(Reservation $reservation): int`

#### 4.3. DashboardService
- [ ] **Fichier:** `app/Services/DashboardService.php`
  - [ ] Remplacer `getTopBiplaceurs()` par `getTopInstructors(?string $activityType = null)`
  - [ ] Remplacer `getFlightStats()` par `getActivityStats(?string $activityType = null)`
  - [ ] Remplacer statistiques "flights" par "sessions"
  - [ ] Utiliser `ActivitySession` au lieu de `Reservation`
  - [ ] Grouper par `activity_type` au lieu de `flight_type`

#### 4.4. Tests
- [ ] Mettre à jour tests pour tous les services modifiés
