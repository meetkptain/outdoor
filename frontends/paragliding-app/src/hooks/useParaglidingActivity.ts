...
import { PARAGLIDING_ACTIVITY_ID } from '../config/env'
...
export function useParaglidingActivity() {
    ...
    async function fetchActivity() {
      ...
      try {
        const response: AxiosResponse<ActivityResponse> = await apiClient.get(
          `/activities/${PARAGLIDING_ACTIVITY_ID}`,
        )
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Impossible de récupérer l'activité de parapente",
        )
      }
    }
